# Generated by Django 3.0.8 on 2020-12-15 13:31

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('app_user', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='SupplychainModel',
            fields=[
                ('supplychain_id', models.AutoField(primary_key=True, serialize=False)),
                ('supply_chain_name', models.TextField()),
                ('shipping_transportation_routes_information', models.TextField()),
                ('gathering_data', models.TextField()),
                ('social_impact', models.TextField()),
                ('final_remarks', models.TextField(blank=True, null=True)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now_add=True)),
                ('updated_by', models.IntegerField(default=1)),
                ('deleted', models.IntegerField(default=0)),
                ('user_id', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='app_user.AppUserModel')),
            ],
            options={
                'db_table': 'supplychain',
            },
        ),
    ]
